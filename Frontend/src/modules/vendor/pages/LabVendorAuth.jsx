...
export default function LabVendorAuth() {
  ...
  const validateForm = () => {
    ...
    if (tab === 0) {
      ...
      if (!fullName) errs.fullName = "Lab Name is required";
      if (!email) {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      dispatch(vendorLoginSuccess({ 
        user: { name: 'Lab User', email: emailOrPhone }, 
        token: 'dummy-token' 
      }));
        ...
        <div className="z-10 flex flex-col gap-6 items-center text-center max-w-md mx-auto">
          ...
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#319C9B]/30 rounded-full border border-[#319C9B]/50 mb-4 backdrop-blur-md">
              <span className="text-white text-[10px] font-black tracking-widest uppercase">Laboratory Partner Portal</span>
            </div>
            <h2 className="text-3xl font-black text-white tracking-tight leading-tight">
              Manage your diagnostic center with E Mediclub
            </h2>
            <p className="text-white/70 font-medium text-sm mt-4 leading-relaxed">
              Expand your reach. Manage home sample collections, process lab test orders, and upload reports seamlessly.
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-100 rounded-[32px] p-6 sm:p-8 shadow-premium relative">
            ...
            <div className="mb-6">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-wide">
                {tab === 0 ? "Welcome Back!" : "Join as a Lab Partner"}
              </h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-wider">
                {tab === 0 ? "Sign in to your diagnostic panel" : "Quickly setup your laboratory account"}
              </p>
            </div>
              ...
              {tab === 0 ? (
                ...
                <>
                  <AuthInput
                    ...
                    type="text"
                    placeholder="lab@example.com or 8888888888"
                    icon={FiMail}
                  ...
                  <div className="flex justify-end -mt-2">
                    <Link to="/vendor/lab/forgot-password" className="text-[10px] font-black text-[#319C9B] uppercase hover:underline">
                      Forgot Password?
                    </Link>
                  </div>
                </>
